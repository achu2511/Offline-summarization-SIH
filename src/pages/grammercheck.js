...
function Grammer() {
    ...
    const handleSummarize = () => {
      ...
      axios.post('http://127.0.0.1:5000/grammercheck', data)
        .then((response) => {
          // Handle the response from the Flask backend, e.g., display the summarized text
          console.log("that"+ response.data.grammer_check);
          // document.getElementById('grammercheck').innerText=response.data.grammer_check
          setOutput2({
            output2:response.data.grammer_check
          })
  ...
  <u><h3 style={{ textAlign: 'center' }}>Corrected Content</h3></u>
